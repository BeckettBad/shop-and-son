...
> **ACTIVE SUB-TASK: K3 — Product detail page at `/product/?handle=<handle>`** (see the K3 section below + the plan-branch K3 buy-out-link revision). K2 GATE PASSED (342=342, 0 imageless after Ben's admin fix; pushed to origin/dev). Order: K3 → L4 → K4 (verify-gate) → K6 → K5. Uses K1 getProduct + the shared formatMoney + sanitizeShopifyHtml gate for descriptionHtml. Wave 3 + K1 live on main via PR #7.
