...
> **ACTIVE SUB-TASK: K7 — Availability sort (in-stock first, sold-out last) in every catalogue** (see the K7 section below). Sort in renderCatalogRows (the single render-prep point → snapshot + live refresh agree). Then K5 (deploy.yml; Actions Variables already set per operator). Then Wave 2 ship gate.
