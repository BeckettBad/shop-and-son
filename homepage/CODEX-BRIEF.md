...
> **ACTIVE SUB-TASK: (none — K2 c68cfc9 + K2-fix 76ebf36 on dev, built green + live path hand-verified. PAUSED at the K2 verify-gate for operator (catalogue visuals + live refresh: prices $295 not $295.0, clothing images persist after refresh, sold-out correct, refresh a no-op when unchanged). Next per order: K3 → L4 → K4 (verify-gate) → K6 → K5. FLAG: clothing collections have no images on the Headless channel — see K2-fix log. Wave 3 + K1 live on main via PR #7 (left live per operator).)**
