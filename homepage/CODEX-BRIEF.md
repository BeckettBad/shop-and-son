...
> **ACTIVE SUB-TASK: (none — K6 3dc8053 done & pushed to origin/dev. ⚠️ OBJECTS now mirrors the live nav = 4 BRANDS, not LIVING/KITCHEN/LIBRARY/SEATING — awaiting operator call (accept / add categories to Shopify admin / exempt OBJECTS). NEXT: K5 (nightly rebuild cron + PUBLIC_ token into the Pages deploy build — touches .github/workflows/deploy.yml, operator-aware) — the LAST Wave-2 sub-task.)**
