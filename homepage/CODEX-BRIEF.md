...
> **ACTIVE SUB-TASK: (none — K4 6b4710e done & pushed to origin/dev; live cart pipeline proven via API (cartCreate→cartLinesAdd→line+checkoutUrl). PAUSED at the K4 VERIFY-GATE for operator hand-check: add-to-cart → drawer → Shopify checkout, stepper/remove/badge/reload-persist. Then K6 → K5.)**
...
- 2026-07-02 — Phase K6: live hero nav menus mirror Shopify admin — build:green check:green — added src/lib/menu.ts helper using existing Storefront getMenu("main-menu") only; records mapping in code: CLOTHES CATEGORIES ← clothing children minus shop all, CLOTHES DESIGNERS ← designers children, OBJECTS leaves ← objects children, with SHOP ALL parents staying clothing-1/house-1. HeroVideo.astro now hydrates once on load, no-ops on missing/empty menu/token, equality-guards each list before replacing, preserves section/subgroup DOM state, rebuilds collection leaf buttons with the same dash/bullet markup/data attributes, and switches [data-shop-all] catalogue opening to a single delegated .hero__menu listener so hydrated entries keep working. content.ts untouched as pre-hydrate/no-token fallback. Ready for operator verify.
