...
- **Shopify Storefront API** (read-only) for live product data — see `homepage/src/lib/shopify.ts`.

## Structure

```
homepage/                      THE homepage — the live site (active work)
├── src/
│   ├── components/
│   │   ├── TopBar.astro          nav + live clock + dusk/index toggles
│   │   ├── IndexOverlay.astro    the "+ index" running catalog
│   │   └── blocks/               the homepage blocks (About, Clothing,
│   │       │                     Objects, Music, Vault, Preorders)
...
│   public/images/blocks/         homepage photography
├── CODEX-BRIEF.md             active hand-off brief (Claude → Codex)
└── SESSION-BRIEF.md           start-of-session shared context
public/preorders/              self-contained preorders site, bundled at /preorders/
archive/                       out-of-scope, gitignored, reference only:
                               retired v1, shelved homepage-v3, raw asset
                               masters (assets-src), design refs, scratch docs
```

**Editing content:** words and curation live in `homepage/src/data/content.ts`.
Live products/photos/prices come from Shopify once configured — the owner's
normal "add a product in Shopify admin" flow surfaces them here, no code changes.
...
- **`dev`** — where features are built and tested **locally** (from `homepage/`:
  `npm run dev`, `npm run build`, `npm run astro check`). Pushing `dev` does
  **not** deploy.
- **`main`** — production. The GitHub Actions workflow builds `homepage/` and
...
Copy `homepage/.env.example` to `homepage/.env` and fill in your Storefront API
token. Until then the site builds and renders from `homepage/src/data/content.ts`.
See `homepage/src/lib/shopify.ts` for the data shape and how curatorial fields
map to Shopify metafields (`editorial` namespace).
