import { decimalToMinor } from "./shopify-analytics";

interface LegacyShopifyMetric {
  currency: string;
  date: string;
...
function normalizeLegacyResponse(value: unknown, currency: string, timezone: string): LegacyShopifyMetric[] {
...
export async function syncAllChannelShopifyAnalytics(
  db: D1Database,
...
WITH CURRENCY '${options.currency}', TIMEZONE '${options.timezone}'`;
  ...
  try {
    if (!/^[a-z0-9][a-z0-9-]*\.myshopify\.com$/.test(options.shopDomain)) throw new Error("Invalid Shopify shop domain");
    ...
    const tokenResponse = await fetcher(`https://${options.shopDomain}/admin/oauth/access_token`, {
    ...
    if (typeof tokenPayload.scope !== "string" || !tokenPayload.scope.split(",").map((scope) => scope.trim()).includes("read_reports")) {
      throw new Error("Shopify token lacks read_reports scope");
    }

    const response = await fetcher(`https://${options.shopDomain}/admin/api/2026-07/graphql.json`, {
    ...
    const metrics = normalizeLegacyResponse(await response.json(), options.currency, options.timezone);
